...
import { COMPONENT_PROPS, type AtomicLayer } from './component-props'
...
// The picker groups components by atomic level, lowest first.
const LAYERS: { layer: AtomicLayer; label: string }[] = [
  { layer: 'atom', label: 'Atom' },
  { layer: 'molecule', label: 'Molecule' },
  { layer: 'organism', label: 'Organism' },
  { layer: 'pattern', label: 'Pattern' },
]
const simulatorGroups = LAYERS.map(({ layer, label }) => ({
  layer,
  label,
  tags: simulatorTags.filter((tag) => COMPONENT_PROPS[tag]!.layer === layer),
          ...
          <div class="pg-section__preview">
            <div class="pg-sim-picker">
              ...
              <select id="pg-sim-picker" v-model="simulatorTag" class="pg-sim-picker__select">
                <optgroup
                  v-for="group in simulatorGroups"
                  :key="group.layer"
                  :label="`${group.label} (${group.tags.length})`"
                >
